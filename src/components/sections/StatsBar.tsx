  ...
  return (
    <section ref={ref} className="bg-brand-charcoal border-y border-brand-gold/10">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 py-12 sm:py-16">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-10 lg:gap-0 lg:divide-x lg:divide-brand-gold/10">
          {stats.map((stat, i) => (
            <motion.div
              key={stat.label}
            ...
            >
              <span className="font-display text-4xl sm:text-5xl lg:text-6xl font-semibold text-brand-gold">
                <CountUp target={stat.value} suffix={stat.suffix} active={inView} />
              </span>
              <span className="font-body text-[10px] sm:text-sm text-brand-muted mt-2 tracking-wide uppercase">
                {stat.label}
              </span>
            </motion.div>
          ))}
